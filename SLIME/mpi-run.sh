...
#    lines=$(sort $HOST_FILE_PATH|uniq|wc -l)
  ...
  time mpirun -quiet --mca btl_tcp_if_include eth0 --allow-run-as-root -np ${AWS_BATCH_JOB_NUM_NODES} --hostfile combined_hostfile /SLIME/bin/slime  -c=${NUM_PROCESSES} -t=28800 -d=7 test.cnf
}
